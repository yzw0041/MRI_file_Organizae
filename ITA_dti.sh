...
cat "anat.txt" | while read C; do
       ...
       if [ -d "$sub" ]; then
          echo "yes"
          #mkdir $new_path/$sub
          if [ -d "$new_path/$sub/anat" ]; then
              cp $C  $new_path/$sub/anat/
          else
              mkdir -p $new_path/$sub/anat  # -p can recursively make a new directory ;
              cp $C  $new_path/$sub/anat

          fi
          # count how many files inside this folder
          count=`ls $new_path/$sub/anat/anat*.nii.gz | wc -l`
          echo $(( count++))
          cp  $new_path/$sub/anat/$ff  $new_path/$sub/anat/anat$count.nii.gz
          echo $new_path/$sub/anat/anat$count.nii.gz >> $new_path/anatomical_input.txt
          rm $new_path/$sub/anat/$ff
